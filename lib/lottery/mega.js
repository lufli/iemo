...
function standardizeData(rawData) {
  ...
  const {
    N1, N2, N3, N4, N5, MBall, Megaplier,
  } = dataJson.Drawing;
  const finalResult = {
    type: 'mega',
    N1,
...
function checkNumbersForPrize(winningNumber, playNumber) {
  ...
  const multiplier = playNumber.power ? winningNumber.multiplier : 1;
  winningResult.prize = calculatePrize(numberOfWhiteBallHits, specialHit, multiplier);
